...
import { X, Trophy, Loader2, Send, CheckCircle2, AlertCircle } from 'lucide-react';
...
export default function TournamentEmailModal({ leagueId, onClose }: Props) {
  ...
  // (Re)load the preview whenever the note changes.
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    setError(null);
    ...
    (async () => {
      try {
        const res = await fetch(`/api/leagues/${leagueId}/jtt/tournament-email`, {
          method: 'POST',
    ...
    // recipientsTouched intentionally omitted: only refetch on note change.
    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-stretch sm:items-center justify-center p-0 sm:p-4">
      <div className="bg-white w-full sm:max-w-4xl sm:rounded-2xl shadow-xl flex flex-col max-h-screen sm:max-h-[92vh] overflow-hidden">
        ...
        <div className="flex items-center gap-2 px-4 sm:px-6 py-3 border-b border-gray-200">
          ...
          <h2 className="font-semibold text-gray-900 flex-1">Email tournament sign-ups</h2>
        ...
        <div className="flex flex-col lg:flex-row flex-1 min-h-0">
          ...
          <div className="lg:w-80 shrink-0 border-b lg:border-b-0 lg:border-r border-gray-200 p-4 space-y-4 overflow-y-auto">
            <p className="text-xs text-gray-500">
              Sends the four season-end division sign-up links to your coaches to forward to their players.
            </p>
            ...
            <div>
              ...
              <div className="flex items-center justify-between mt-1">
                <p className="text-[11px] text-gray-400">
                  Comma or line separated. Shares the saved coach list with the results email.
                </p>
                <button
                  onClick={saveDefaults}
                  disabled={savingDefaults || parsedRecipients.length === 0}
                  className="text-[11px] font-medium text-emerald-600 hover:text-emerald-700 disabled:opacity-40 whitespace-nowrap"
                >
                  {savingDefaults ? 'Saving…' : 'Save as default'}
                </button>
              </div>
              {savedNote && <p className="text-[11px] text-green-600 mt-1">{savedNote}</p>}
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                placeholder="Great season, everyone — let's finish strong!"
                style={{ color: '#111827', backgroundColor: '#fff' }}
            ...
            <button
              ...
              className="w-full inline-flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white font-medium rounded-lg px-4 py-2.5 text-sm"
            >
          ...
          <div className="flex-1 min-h-0 flex flex-col bg-gray-100">
            ...
            <div className="flex-1 min-h-0 relative">
              ...
              {html ? (
                <iframe
                  title="Tournament email preview"
                  srcDoc={html}
