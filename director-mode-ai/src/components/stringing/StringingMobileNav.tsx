...
import { Wrench, ClipboardList, Users, Package, Zap, Menu, X } from 'lucide-react';

interface StringingMobileNavProps {
  userName: string;
  userInitial: string;
}

export default function StringingMobileNav({ userName, userInitial }: StringingMobileNavProps) {
    ...
    <>
      <header className="md:hidden fixed top-0 left-0 right-0 h-16 bg-[#002838] border-b border-white/[0.06] flex items-center justify-between px-4 z-50">
        <Link href="/" className="flex items-center gap-2">
          ...
          <span className="font-display text-base text-white">StringingMode</span>
        </Link>
          ...
          <div className="absolute top-16 right-0 w-64 bg-[#002838] h-[calc(100vh-4rem)] border-l border-white/[0.06] overflow-y-auto">
            <div className="p-4 border-b border-white/[0.06]">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-pink-400/10 flex items-center justify-center">
                  <span className="text-pink-400 font-semibold">{userInitial}</span>
                </div>
            ...
            <nav className="p-3">
              <ul className="space-y-1">
                <MobileNavItem href="/stringing/jobs" icon={ClipboardList} onClick={() => setIsOpen(false)}>Job Board</MobileNavItem>
                <MobileNavItem href="/stringing/customers" icon={Users} onClick={() => setIsOpen(false)}>Customers</MobileNavItem>
                <MobileNavItem href="/stringing/catalog" icon={Package} onClick={() => setIsOpen(false)}>String Catalog</MobileNavItem>
              </ul>
            </nav>
