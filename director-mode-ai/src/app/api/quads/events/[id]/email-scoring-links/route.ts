/**
 * POST /api/quads/events/[id]/email-scoring-links
 *
 * Director-only. Sends every confirmed entry (position='in_flight') a
 * personal scoring URL — `${origin}/quads/player/${entry.player_token}` —
 * via email. Each player can use their link to enter scores for any of
 * their matches.
 *
 * Returns: { sent: number, skipped_no_email: number, skipped_unsubscribed: number }
 */
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: flights } = flightIds.length
    ? await admin.from('quad_flights').select('id, name').in('id', flightIds)
    : { data: [] as any[] };
  const flightById = new Map(((flights as any[]) || []).map((f) => [f.id, f.name]));
  ...
  for (const e of list) {
    ...
    const result = await sendQuadsScoringLinkEmail({
      ...
      flightName: flightById.get(e.flight_id) ?? null,
      scoringUrl: `${origin}/quads/player/${e.player_token}`,
    });
    if (result.sent) sent++;
    else if (result.reason === 'unsubscribed') skipped_unsubscribed++;
    else skipped_no_email++; // count error-fails here for simplicity
  }
