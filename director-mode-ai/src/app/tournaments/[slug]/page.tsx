...
export default async function PublicTournamentLandingPage({
  params,
  searchParams,
}: {
  ...
  if (!ev || !VALID.has((ev as any).match_format)) return notFound();
  const e = ev as any;
    ...
    <div className="min-h-screen bg-[#001820] text-white">
      <header className="border-b border-white/10">
        <div className="max-w-3xl mx-auto px-4 py-6 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-xs text-white/40">{FORMAT_LABELS[e.match_format]}</div>
      ...
      <main className="max-w-3xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="bg-white/5 rounded-xl p-5 space-y-2 text-sm text-white/80">
          <div className="font-semibold text-white">Format: {FORMAT_LABELS[e.match_format]}</div>
          ...
          {(waitlistCount ?? 0) > 0 && (
            <p className="text-amber-200">
              Waitlist: {waitlistCount} {isDoubles ? 'pair' : 'player'}
              {waitlistCount === 1 ? '' : 's'}. Registering now will add you to the waitlist.
            </p>
          )}
        </div>
