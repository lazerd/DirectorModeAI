...
export default function RegisterForm({ slug, feeCents, ageMax, genderRestriction, isDoubles }: Props) {
    ...
    try {
      const res = await fetch('/api/tournaments/register', {
        ...
        body: JSON.stringify({
          ...
          partner_ntrp: form.partner_ntrp ? parseFloat(form.partner_ntrp) : null,
        }),
      });
      ...
      window.location.href = `/tournaments/${slug}/registered?entry=${data.entry_id}`;
    } catch (err: any) {
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        <div className="sm:col-span-2">
          <label className="block text-xs font-medium text-gray-700 mb-1">
            {isDoubles ? 'Captain Name' : 'Player Name'} *
          </label>
          <input
            type="text"
            required
        ...
        {isDoubles && (
          <>
            <div className="sm:col-span-2 pt-2 border-t border-gray-200">
              <p className="text-xs text-gray-600 font-medium">Doubles Partner *</p>
            </div>
            <div>
            ...
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">Partner Email</label>
              <input
                type="email"
            ...
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">Partner Phone</label>
              <input
                type="tel"
            ...
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">Partner NTRP</label>
              <input
                type="number"
                min="1"
                max="7"
                step="0.5"
                value={form.partner_ntrp}
                onChange={(e) => setForm({ ...form, partner_ntrp: e.target.value })}
                className="w-full px-3 py-2 border rounded-lg text-gray-900"
              />
            </div>
          </>
