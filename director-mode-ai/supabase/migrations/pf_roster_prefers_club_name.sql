-- The roster calls people what the CLUB calls them.
--
-- pf_vault_user_link.sql made the roster read the club's PlayerVault row for
-- the contact address ("a director who fixes an address in the club's own
-- records is telling us where to write") but left the NAME resolving the other
-- way round: profile first, roster row last. So the name was the one field a
-- director could correct in PlayerVault and watch nothing happen - which is
-- exactly what happened to Walden Browne, filed under his legal first name.
--
-- Only the full_name expression changes; everything else here is
-- pf_vault_user_link.sql verbatim, so the two files agree wherever they
-- overlap and either may be re-run.
--
-- Safe to re-run.
