...
-- ============================================================================
-- 7. tournament_tables.sql — generic tournament_entries + tournament_matches
--    tables for non-Quads formats (RR, Single Elim, FMLC, FFIC). Shares the
--    `events` row + per-event extensions from quads.sql; doesn't duplicate.
-- ============================================================================
...
CREATE TRIGGER trg_tournament_entries_touch
  BEFORE UPDATE ON tournament_entries FOR EACH ROW EXECUTE FUNCTION touch_tournament_updated_at();

DROP TRIGGER IF EXISTS trg_tournament_matches_touch ON tournament_matches;
CREATE TRIGGER trg_tournament_matches_touch
  BEFORE UPDATE ON tournament_matches FOR EACH ROW EXECUTE FUNCTION touch_tournament_updated_at();
...
-- ============================================================================
-- 10. swim.sql — SwimMode (volunteer points tracker for swim teams)
-- ============================================================================
